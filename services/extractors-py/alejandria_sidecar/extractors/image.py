"""Image extractor wrapper for the sidecar CLI."""
...
def extract_image(path: Path) -> dict[str, Any]:
    """Run the MVP image extractor and return a JSON-ready dict."""
    if not path.exists():
        return file_unreadable_envelope(path)
    try:
        from alejandria.extractors.image import ImageExtractor  # type: ignore[import-not-found]

        metadata = ImageExtractor().extract(path)
