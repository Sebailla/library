...
The EPUB wrapper is the entry point used by the sidecar CLI to convert
an :class:`ExtractedMetadata` (from the MVP ``alejandria.extractors.epub``
module) into a JSON-friendly dict the CLI emits on stdout.

These tests drive the wrapper through its public function
:func:`extract_epub` and assert the contract the spec promises:

* JSON envelope carries ``schema_version: 1``.
* The wrapped payload contains ``title``, ``author``, ``format: "epub"``
  and a non-empty ``extracted_text`` when the fixture's spine is valid.
...
def test_extract_epub_returns_envelope_for_real_fixture(minimal_epub: Path) -> None:
    """Happy path: a minimal valid EPUB must yield title + author + body text."""
    ...
    # The fixture's chapter body must surface in the wrapper's text field
    # so the FTS5 index has something to search.
    assert envelope["extracted_text"]
    assert "Sample chapter body" in envelope["extracted_text"]
    # The wrapper always declares which extractor produced the envelope.
    assert envelope["extractor_name"] == "epub"


def test_extract_epub_missing_file_returns_file_unreadable(tmp_path: Path) -> None:
...
def test_extract_epub_corrupt_file_surfaces_warning(tmp_path: Path) -> None:
    """Corrupt (non-ZIP) files must NOT crash the wrapper.

    The MVP :class:`EpubExtractor` catches ZIP parse failures and
...
def test_cli_extract_dispatches_epub(minimal_epub: Path) -> None:
    """The CLI's ``extract`` subcommand must dispatch ``.epub`` to the EPUB wrapper."""
